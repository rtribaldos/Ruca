...
public class Display extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
		...
		if (blobKeyString == null || blobKeyString.equals("")) {
			resp.sendRedirect((new StringBuilder("/?error=")).append(URLEncoder.encode("BlobKey not provided", "UTF-8"))
					.toString());
			return;
		}
		...
		if (!result.isPublic() && !result.getOwner().equals(user)) {
			resp.sendRedirect((new StringBuilder("/?error="))
					.append(URLEncoder.encode("Not authorized to access", "UTF-8")).toString());
			return;
		} else {
			String rotation = req.getParameter("rotate");
			String displayURL = (new StringBuilder(String.valueOf(result.getURLPath()))).append("&rotate=")
